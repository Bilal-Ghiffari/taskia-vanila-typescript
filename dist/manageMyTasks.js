document.addEventListener("DOMContentLoaded", () => {
    ...
    const existingTasks = myTasks.getTasksUser();
    const myTaskWrapper = document.getElementById("myTaskWrapper");
    ...
    const displayAllTasks = (task = existingTasks) => {
        if (task.tasks.length <= 0) {
            myTaskWrapper.className = "hidden";
        }
        else {
            myTaskWrapper.innerHTML = "";
          ...
          <div class="task-card flex flex-col gap-5">
              <div class="flex gap-3 items-center">
                  ...
                  <div class="flex flex-col">
                      <p class="font-bold text-lg leading-[27px]">${capitalizeFirstChar(element.taskName)}</p>
          ...
          ${element.isCompleted
                    ? `
          <a href="#" id="completedTask-${element.id}"
          class="hidden"></a>
          `
                    : `
      <a href="#" id="completedTask-${element.id}"
      ...
      `;
                myTaskWrapper.appendChild(itemTask);
                itemTask
                    .querySelector(`#completedTask-${element.id}`)
                    .addEventListener("click", (event) => {
                    event.preventDefault();
                    myTasks.updateComplateTask(element.id);
                    const updateTask = myTasks.getTasksUser();
...
//# sourceMappingURL=manageMyTasks.js.map